...
// ---- VENDOR PUBLIC KEY ----
// This is the public key corresponding to the private signing key in license-vendor.mjs.
...
export function normalizeFingerprint(fp) {
  return String(fp || "").replace(/[^A-Fa-f0-9]/g, "").toLowerCase();
}

export function fingerprintMatches(expected, actual) {
  expected = normalizeFingerprint(expected);
  actual = normalizeFingerprint(actual);
  if (!expected || !actual) return false;
  if (expected.length === actual.length) return expected === actual;
  if (expected.length === 20) return actual.startsWith(expected);
  return false;
}
...
export function verifyLicense(licenseObj, opts = {}) {
  try {
    ...
    // machine binding: accept either full fingerprint or the short Machine ID.
    const here = opts.fingerprint || machineFingerprint();
    if (!fingerprintMatches(d.fp, here)) return { ok: false, reason: "wrong-machine", boundTo: d.fp };
